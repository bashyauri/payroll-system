<div>
    <div class="flex items-center justify-between p-4">
        <flux:heading size="lg">Employee deduction</flux:heading>
        ...
        <div class="flex items-center gap-4">
            ...
            {{-- <div class="relative">
                ...
                <flux:input wire:model.live="search" placeholder="Search deductions..." class="pl-10 w-64" />
            </div> --}}
            ...
            <!-- Trigger Button -->
            <flux:modal.trigger name="add-deduction">
            ...
            <!-- Modal -->
            <flux:modal name="add-deduction" class="md:w-[800px]">
                <div class="space-y-6">
                    <div>
                        <flux:heading size="lg">Add New Deduction</flux:heading>
                        <flux:text class="mt-2 text-gray-600 dark:text-gray-400">Assign deduction to an employee
                        </flux:text>
                        <x-action-message class="me-3" on="deduction-added" />
                    </div>
                    ...
                    <form wire:submit.prevent="saveDeduction">
                        ...
                        <!-- Deduction Type -->
                        <div class="mb-6">
                            <flux:select label="Deduction Type" wire:model="deduction_type_id" class="w-full"
                            ...
                            @error('deduction_type_id') <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="flex items-center justify-between pt-6 mt-6 border-t border-gray-200">
                            <flux:button variant="ghost" type="button"
                                x-on:click="$flux.modal('add-deduction').close()">
                                Cancel
                            </flux:button>
    ...
    <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
        ...
        <x-action-message
            ...
            on="deduction-added">
            {{ __('Deduction added successfully!') }}
        </x-action-message>
        <!-- Success Message -->
        <x-action-message
            class="bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-200 px-4 py-3 rounded-md"
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-800">
                    <tr>
                        ...
                        <th scope="col"
                            ...
                            <div class="flex items-center gap-1">
                                Deduction Type
                                <button type="button" wire:click="sort('deduction_type.name')"
                                    class="text-gray-400 hover:text-gray-500">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse ($this->deductions as $deduction)
                        @include('livewire.admin.employees.modals.edit-deductions')
                        @include('livewire.admin.employees.modals.delete-deduction')
                        <tr wire:key="deduction-{{ $deduction->id }}"
                                ...
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10">
                                        <img class="h-10 w-10 rounded-full"
                                            src="{{ $deduction->employee->user?->avatar_url ?? 'https://ui-avatars.com/api/?name=' . urlencode($deduction->employee->user?->name ?? '') . '&background=random' }}"
                                            alt="{{ $deduction->employee->user?->name ?? '' }}">
                                    </div>
                                    <div class="ml-4">
                                        <div class="text-sm font-medium text-gray-900 dark:text-white">
                                            {{ $deduction->employee->user?->name ?? 'N/A' }}
                                        </div>
                                        <div class="text-sm text-gray-500 dark:text-gray-400">
                                            {{ $deduction->employee->user?->email ?? 'N/A' }}
                                        </div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <!-- Amount -->
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                {{ config('app.currency') }}{{ number_format($deduction->amount, 2) }}
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center space-x-2">
                                    <flux:modal.trigger :name="'edit-deduction-'.$deduction->id">
                                        <button wire:click="editDeduction({{ $deduction->id }})"
                                    ...
                                    <flux:modal.trigger :name="'delete-deduction-'.$deduction->id">
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-4 text-center text-sm text-gray-500 dark:text-gray-400">
                                <div class="flex flex-col items-center justify-center py-8">
                                    ...
                                    <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">No deductions found
                                    </h3>
        ...
        @if($this->deductions->hasPages())
            <div
                ...
                <div class="flex-1 flex justify-between sm:hidden">
                    ...
                    @if($this->deductions->hasMorePages())
                ...
                <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                    <div>
                        <p class="text-sm text-gray-700 dark:text-gray-300">
                            Showing
                            <span class="font-medium">{{ $this->deductions->firstItem() }}</span>
                            to
                            <span class="font-medium">{{ $this->deductions->lastItem() }}</span>
                            of
                            <span class="font-medium">{{ $this->deductions->total() }}</span>
                            results
                        </p>
                    </div>
                    <div>
                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                            ...
                            @foreach($this->deductions->getUrlRange(1, $this->deductions->lastPage()) as $page => $url)
                                @if($page == $this->deductions->currentPage())
